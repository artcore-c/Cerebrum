...
# Green divider for output sections
green_bar() {
    echo -e "${GREEN}$(printf '%*s' "$TERM_WIDTH" '' | tr ' ' '-')${NC}"
}
...
# Center with manual padding (for lines with special UTF-8 chars)
center_manual() {
    local s="$1"
    local padding="$2"  # Manual adjustment for special characters
    printf "%*s\n" $(( (TERM_WIDTH + ${#s}) / 2 + padding )) "$s"
}
...
show_banner() {
    ...
    echo -e "${MAGENTA}"
    center "Distributed Interactive AI Code Assistant"
    echo -e "${GRAY}"
    center_manual "Cerebrum™ © 2025 Robert Hall. All rights reserved" +3
    echo
}
...
generate_stream() {
    ...
    while IFS= read -r line; do
        ...
        if echo "$payload" | jq -e '.token' >/dev/null 2>&1; then
            echo -n "$(echo "$payload" | jq -r '.token')"  # Flush EVERY token
            
        elif echo "$payload" | jq -e '.done' >/dev/null 2>&1; then
            token_count=$(echo "$payload" | jq -r '.total_tokens // 0')
            elapsed=$(echo "$payload" | jq -r '.inference_time // 0')
            echo
            break
    ...
    echo
    green_bar
    # Format stats in gray
    echo -e "${GRAY}[$token_count tokens, ${elapsed}s, vps]${NC}"
    echo
}
